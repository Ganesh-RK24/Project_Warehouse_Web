...
def save_csv_by_wh(file_stream, vendor_path):
    ...
    for row_num, row in enumerate(reader, start=2):
        ...
        try:
            ...
            # Could not parse date; skip row but flash debug info
            print(f"DEBUG: Skipping row {row_num} due to invalid date format: '{date_raw}'")
            continue
    ...
    for wh, rows in wh_groups.items():
        ...
        print(f"DEBUG: Saved file for WH '{wh}': {save_path}")
...
def log_upload(user_email, filename, warehouse_list):
    ...
    print(f"DEBUG: Logged upload for user '{user_email}' with file '{filename}' and warehouses: {warehouse_list}")
...
def upload_file():
    ...
    if not user_info:
        ...
        print(f"DEBUG: Unauthorized access attempt by {email}")
    ...
    if request.method == 'POST':
        if 'file' not in request.files or request.files['file'].filename == '':
            ...
            print("DEBUG: No file uploaded in POST request.")
        ...
        print(f"DEBUG: Received upload request from user {email}")
        print(f"DEBUG: Entry dates: {dates}")
        print(f"DEBUG: Warehouse Nos: {warehouse_nos}")
        print(f"DEBUG: Manpower counts: {manpower_counts}")

        if not dates or not warehouse_nos or len(dates) != len(warehouse_nos):
            flash("❌ Please fill all Date and Warehouse No fields.")
            print("DEBUG: Date and Warehouse No fields validation failed.")
            return redirect(request.url)

        for d, wh in zip(dates, warehouse_nos):
            if d.strip() == '' or wh.strip() == '':
                flash("❌ Date and Warehouse No fields cannot be empty.")
                print("DEBUG: Empty Date or Warehouse No found in form data.")
                return redirect(request.url)

        # Fill missing manpower entries with empty strings to match lengths
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                flash(f"✅ File uploaded and saved under warehouses: {', '.join(wh_list)}")
                flash(f"✅ Manpower details saved.")
                print(f"DEBUG: Upload completed successfully for user {email}")
            ...
            print(f"DEBUG: Uploaded file not allowed or missing: {file.filename if file else 'No file'}")
...
def upload_from_url():
    ...
    try:
        ...
        saved_files = save_csv_by_wh(file_stream, vendor_path)
        wh_list = [wh for wh, _ in saved_files]
        log_upload(email, filename, wh_list)

        print(f"DEBUG: File uploaded from URL successfully by user {email}")

        return jsonify({"message": f"File uploaded successfully under warehouses: {', '.join(wh_list)}"})

    except Exception as e:
        print(f"DEBUG: Exception in upload_from_url: {e}")
