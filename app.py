...
SMTP_PORT = 587
SENDER_EMAIL = "ganeshan@abfrl.adityabirla.com"
SENDER_PASSWORD = "hqlwcwvfcbcbykvt"
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                # Save manpower details CSV in vendor folder (code omitted here for brevity)

                flash(f"✅ File uploaded and saved under warehouses: {', '.join(wh_list)}")

                # Send email with each saved CSV as attachment to the logged-in user's email
                for wh, path in saved_files:
                    subject = f"Warehouse Data Uploaded: {wh}"
                    body = f"Dear {user_info['name']},\n\nPlease find attached the CSV file for warehouse {wh}.\n\nBest Regards,\nAudit Team"
                    send_email(email, subject, body, path)

                flash("✅ Email(s) sent with uploaded CSV file(s).")
